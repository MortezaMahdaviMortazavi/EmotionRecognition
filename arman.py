...
def make_grad(module,set=False):
...
class XLMRobertaClassifier(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv1d = nn.Conv1d(self.roberta.config.hidden_size, 128, kernel_size=3)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        outputs = outputs.permute(0, 2, 1)  # Reshape for Conv1d
        outputs = self.conv1d(outputs)
        outputs = self.maxpool(outputs)
        outputs = torch.squeeze(outputs, dim=2)  # Remove the last dimension
        outputs, _ = self.lstm(outputs)
        outputs = torch.mean(outputs, dim=1)  # Apply mean pooling along the sequence
...
# model = XLMRobertaForSequenceClassification('xlm-roberta-large',num_label=7)


class F1_Loss(nn.Module):
    def __init__(self, num_classes=7,epsilon=1e-7):
        super().__init__()
        self.num_classes = num_classes
        self.epsilon = epsilon

    def forward(self,y_pred,y_true):
        y_true_one_hot = F.one_hot(y_true.to(torch.int64),self.num_classes).to(torch.float32)
...
# model.load_state_dict(torch.load('XLM_LSTMCNN_ROBERTA_PE_weights.pt'))
# model.load_state_dict(torch.load('xlmrobertalarge_gru_model.pth'))
# make_grad(model,False)
# model.linear = nn.Linear(model.xlmroberta.config.hidden_size,7).cuda()
# make_grad(model.linear,True)
...
for _iter in range(num_epochs):
    ...
    torch.save(model.state_dict(),'XLM_Large_conv1.pt')
